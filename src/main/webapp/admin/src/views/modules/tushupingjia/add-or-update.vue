<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="用户评分" prop="yonghupingfen">
          <el-select v-model="ruleForm.yonghupingfen" placeholder="请选择用户评分">
            <el-option
                v-for="(item,index) in yonghupingfenOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="用户评分" prop="yonghupingfen">
	      <el-input v-model="ruleForm.yonghupingfen"
                placeholder="用户评分" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.tushufenlei = true;
          ...
          if(o=='yonghupingfen'){
            this.ruleForm.yonghupingfen = obj[o];
	    this.ro.yonghupingfen = true;
            continue;
          }
          if(o=='pingjianeirong'){
      ...
      this.$http({
		...
		if(json.lianxidianhua!=''&&json.lianxidianhua){
          ...
          this.$message.error(data.msg);
            ...
            this.yonghupingfenOptions = "1,2,3,4,5".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
        url: `tushupingjia/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `tushupingjia/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.tushupingjiaCrossAddOrUpdateFlag = false;
                  this.parent.search();
